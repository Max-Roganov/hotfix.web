...
const Basket = ({ match: { params: { areaId, itemId }}, foodAreas, order }) => {
  ...
  var date = new Date(Date.now() + 900);
  var mindate = date.toJSON().slice(0, -5);
  date = new Date(Date.now() + 86400000 * 3);
  var maxdate = date.toJSON().slice(0, -5);
  ...
  return (
    <div className="Place">
      ...
      <div className="Place__choice">
        ...
        <div className="Place__choice-item">
          <span>Назначить</span>
          <input type='datetime-local' min={mindate} step='60' max={maxdate} value={mindate}
            onFocus={() => {
              setFaster(false);
            }}
            onChange={event => {
